    ...
    <v-form @submit.prevent="editManageLaundrys">
        ...
        <v-card class="mx-auto" max-width="400" elevation="0">
            ...
            <v-container class="pt-10">
                <v-text-field v-model="laundryName" :rules="[rules.required]" color="blue" label="세탁소명"
                    placeholder="세탁소 이름을 입력해주세요" variant="outlined"></v-text-field><br>

                <v-text-field v-model="laundryAddr1" :rules="[rules.required]" color="blue" label="우편번호"
                    placeholder="우편번호를 입력해주세요" variant="outlined"></v-text-field><br>

                <v-text-field v-model="laundryAddr2" :rules="[rules.required]" color="blue" label="주소"
                    placeholder="세탁소 주소를 입력해주세요" variant="outlined"></v-text-field><br>

                <!-- <v-text-field v-model="laundryAddr3" :rules="[rules.required]" color="blue" label="상세 주소"
                    placeholder="세탁소 상세 주소를 입력해주세요" variant="underlined"></v-text-field><br> -->

                <v-row>
                    <v-col cols="6">
                        <v-text-field v-model="openTime" :rules="[rules.required]" color="blue" label="오픈 시간" type="text"
                            variant="outlined"></v-text-field>
                    </v-col>
                    <v-col cols="6">
                        <v-text-field v-model="closeTime" :rules="[rules.required]" color="blue" label="마감 시간" type="text"
                            variant="outlined"></v-text-field>
                    </v-col>
                </v-row><br>

                <v-text-field v-model="tel" :rules="[rules.phoneNumber]" color="blue" label="전화번호 ( - 포함 작성)"
                    placeholder="세탁소 전화번호를 작성해주세요" variant="outlined"></v-text-field><br>

                <v-textarea clearable auto-grow v-model="intro" :rules="[rules.minRules]" color="blue" 
                label="소개글" placeholder="세탁소 소개글을 작성해주세요"
                    variant="outlined"></v-textarea><br>

                <v-textarea clearable auto-grow v-model="notice" :rules="[rules.required]" color="blue" 
                label="공지사항" placeholder="공지사항을 작성해주세요"
                    variant="outlined"></v-textarea><br>
                ...
                <div class="mb-5">
                    ...
                    <div class="photo-container" :class="{ 'no-image': !selectedPhoto }">
                        <v-img :src="selectedPhoto" alt="Selected Photo" v-if="selectedPhoto" />
                        <div v-else>No Image</div>
                    </div>
                </div>

                <v-divider></v-divider>
                <v-btn block color="light-blue-darken-4" size="x-large" variant="tonal" type="submit" @click="editManageLaundrys">수정</v-btn>
            </v-container>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                phoneNumber: value => /^\d{3,4}-\d{3,4}-\d{4}$/.test(value) || '유효한 전화번호 형식으로 입력하세요.',
            },
        }
    },
    async created() {
