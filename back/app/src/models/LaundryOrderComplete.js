...
class LaundryOrderComplete {
    ...
    async addOrderList() {
        ...
        let query = "UPDATE ORDER_LIST SET O_DAY = ?, DELEVERY_ADDRESS = ?, O_REQUEST = ?, U_PHONE =?, O_TIME = ? WHERE O_NUM = ?;";
    ...
    async addOrderCompleteList() {
        const orderNum = this.orderNum;
        let query = "INSERT INTO ORDER_COMPLETE (O_NUM, S_ID, U_ID, O_DAY, U_PHONE, O_REQUEST, DELEVERY_ADDRESS, ORD_DAY, ORD_TIME, O_TIME)\
        SELECT O_NUM, S_ID, U_ID, O_DAY, U_PHONE, O_REQUEST, DELEVERY_ADDRESS, ORD_DAY, ORD_TIME, O_TIME\
        FROM ORDER_LIST\
        WHERE O_NUM = ?;";
